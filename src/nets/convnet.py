...
class ConvNet26x26Encoder(nn.Module):
...
class ConvNet48x48Encoder(nn.Module):
    """ConvNet intended to encode 48x48 input."""
    ...
    def __init__(self, n_features):
        ...
        self.conv1 = init_orthogonal(nn.Conv2d(3, 32, 4, stride=2))
        self.conv2 = init_orthogonal(nn.Conv2d(32, 48, 4, stride=2))
        self.conv3 = init_orthogonal(nn.Conv2d(48, 64, 4, stride=2))
        self.conv4 = init_orthogonal(nn.Conv2d(64, n_features, 4, stride=1))
        self.relu = nn.ReLU(inplace=True)
